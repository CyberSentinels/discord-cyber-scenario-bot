...
def handle_secplus(user_responses):
    # Create a list of question IDs, weighted based on user responses
    weighted_question_ids = []
    for i, question in enumerate(secplusdict):
        # Get the question ID and correct answer
        prefix = "secplus_"  # Unique prefix for CISSP questions
    ...
    if not weighted_question_ids:
        weighted_question_ids = [f"{prefix}{i}" for i in range(len(secplusdict))]

    # Retrieve the selected question
    question = secplusdict[int(question_id.split('_')[1])]
